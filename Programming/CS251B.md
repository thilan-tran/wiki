...
- the **rotary router** is an alternative switch microarchitecture:
    ...
    - avoid deadlocks with buble flow control
...
- **virtual interface architecture (VIA)**:
    ...
    - VIA defines a simple set of operations and follows some basic guidelines:
        - eliminate intermediate data copies by copying directly rather than indirectly through a system bufer
    ...
    - in VIA, each process has the illusion of owning the interface to the network:
        ...
        - the NIC gradually works through the work queue, updating the descriptor and transferring ownership when it is done
...
- with virtual network interfaces, the network interface needs to know the address translations of the virtual addresses:
    ...
    - additionally, the NIC has no control over paging and swapping in the OS, so the memroy buffers must be "pinned" before data transfer can take place
