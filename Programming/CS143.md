...
- a relation $R$ is in **Boyce-Codd normal form (BCNF)** with regard to the set of FDs $F+$ if and only if for every nontrivial functional dependency $(X \to Y) \in F$, $X$ contains a key:
    ...
    - there are other definitions for normal forms eg. third and fourth normal forms
        - but BCNF AKA as 3.5th normal form is the most useful and widely used
...
- whenever there is a BCNF violation, perform *decomposition* until all tables are in BCNF using the following algorithm:
    - for all $R$ in the schema, identify all non-trivial dependencies $X \to Y$ that hold on $R$ and $X$ does not contain a key:
        1. compute $X{+}$
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
    1. check `R` in BCNF with $\{inst\}+ = \{inst, o\textit{ff}ice, fact\}$
        - split `R` into `R1(inst, office, fax)` and `R2(dept, cnum, title, unit, inst)`
    2. check `R1` in BCNF with $\{o\textit{ff}ice\}+ = \{o\textit{ff}ice, fax\}$
        - split `R1` into `R3(office, fax)` and `R4(inst, office)`
    3. check `R2` in BCNF with $\{dept,cnum\}+ = \{dept,cnum,title,unit,inst\}$:
        - note that we do not need to check the FD $inst \to o\textit{ff}ice$ or $o\textit{ff}ice \to fax$ since those do not hold in `R2`
        - LHS is indeed a key
    - decomposing into three tables is now BCNF
        - intuitively, `R3` represents fax machines in an office, while `R4` represents the instructor in an office and `R2` represents course details
...
- in the RDBMS, there are three ways to enforce data validity:
    ...
    2. constraints that generate an error and aborts the offending SQL statement
...
- **referential integrity** involves particular values in one table *referencing* ones in another table:
    ...
    - conditions for foreign keys:
        - the referencing attribute should always exist in the referenced table
...
```
- when can RI be violated eg. when table `E` references a key of `S`?
...
- two modification operations can violate the referential integrity:
    ...
    - note that if referenced attributes are *not* unique, adds complications to `ON DELETE` and `ON UPDATE`
...
-- creating first check or egg from nowhere
...
- **MongoDB** is a database for JSON objects:
    ...
    - the data is stored as a collection of documents:
        - **documents** is essentially a JSON object, like rows in RDBM
        - a **collection** is a group of similar documents, like tables in RDBM
...
- MongoDB aggregates:
    ...
    - made up of various **stages** similar to each individual clause of a SQL statement
        - the overall aggregate **pipeline** is similar to the overall SQL statement
    - common aggregate stages:
        - `$match` performs like the SQL `WHERE`
        - `$group` performs like the SQL `GROUP BY`
        - `$sort` performs like the SQL `ORDER BY`
        - `$limit` performs like the SQL `FETCH FIRST`
        - `$project` performs like the SQL `SELECT`
...
- comparing with RDB:
    ...
    - *cons*:
        ...
        - restructuring or combining data is complex and inefficient
            - could be easily combined in relational models using relational operators
...
- motivation of distributed analytics for non-relational DB using clusters:
    ...
    - want to be able to run multiple queries on *distributed* machines for better performance
        - fast large data analytics
    - transform and load data into RDBMS?
        - data transformation is data-specific and labor intensive
        - going from non-relational DB to distributed RDBMS is thus cost prohibitive
...
- ex. Perform search log analysis:
    - given logs of billions of queries, count the frequency of each query:
        ...
        - log file is spread over many machines, records keyword query and times
    - operation is simple, but how can we run it on thousands of machines in parallel?
        ...
        - first, on each machine, transform the large log file into a certain output
            - eg. queries and their frequencies specific to each machine
...
- generalizing process:
    1. map / transform step:
        ...
        - since each log records the times a query was searched, want to extract only the relevant data
            - thus frequency is always 1 in the transformation
    ...
    2. reduce / aggregate step:
        ...
        - importantly, the grouping stuff can move tuples with the same key to the same machine:
    ...
    - the map and reduction steps are the same, except for the specific mapping and reducing *functions*:
        - can set up a distributed infrastructure that allows programmers to specify mappinga dn reducing functions
        ...
        - this is the **MapReduce** model:
            ...
            - reduce function $r$ where $(k, v1), (k,v2), \ldots \to (k, r(v1, v2, \ldots))$
...
- ex. Build an inverted index from web pages for finding matching documents:
    ...
    2. group by key word and concatenate the indices into a list
...
$$T_A = \textit{seek time} + \textit{rotational delay} + \textit{transfer time}$$
    ...
    - disks typically rotate between 1000 to 15000 rpm
        - ex. with 6000 RPM, the average rotational delay is around 0.005 seconds
    - transfer time is a function of the sector length and RPM
        - ex. with 10000 sectors per track at 6000 rpm the transfer time is $\frac{0.01}{10,000} = 0.001$ ms
...
- handling very long tuples that are larger than a block:
    ...
    - thus, instead we can *split* the tuples so that long attributes are stored separately
        - maintain pointers to the separate blocks the actual large attributes are stored
...
- in a **sparse primary index**:
    ...
    - *pros*:
        - less space required
        - if the index can fit into memory, only requires one disk IO
...
- in a **multi-level index**:
    ...
    - *cons*:
        - more space required than dense or sparse index alone
...
- considerations of the **indexed sequential access method (ISAM)**:
    ...
    - however, when the underlying dataset is dynamic and requires frequent updates:
        - need to reserve overflow blocks for the actual blocks on disk *as well as* the indices
        - with more and more updates, searches and organization become much less efficient wtih many overflow blocks
    - *pros*:
        - simple
        - sequential blocks
    - *cons*:
...
- a B+ tree is a balanced tree:
    ...
    - for non-leaf nodes:
        - pointers point the nodes one level below, rather than tuples:
            ...
            - eg. the pointer between key 23 and 56 points to the node referring to tuples between 23 and 56
...
- search on B+ tree:
    ...
    - to search within a node's pointers, may do binary search or sequential search
        - the in-memory search does not dominate the overall search complexity
...
- inserting on B+ tree:
    ...
    3. in the case of non-leaf overflow:
        ...
        - *move* the middle key up to its parent:
            ...
            - this step can introduce overflow at the parent
...
- deleting on B+ tree:
    ...
    3. otherwise, there is no space to merge the current leaf, so we redistribute tuples from a neighboring leaf:
        ...
        - need to update the key in the parent
    ...
    6. in the case of root underflow:
        - root only holds two pointers, so it will only underflow when it becomes empty
...
- a **hash table** stores key-value pairs with constant lookup:
    ...
    - however, if the hash algorithm is not uniformly distributed
        - we will have poor performance with any hash table due to collisions
...
- key concepts behind **extendable hashing**:
    1. use $i$ of $b$ bits from the hash output, increasing $i$ as needed
        - $i$ starts at 0
    2. add an additional level of indirection:
        - $i$ bits of the hashed key are used to index into a directory holding multiple pointers to hash buckets
...
- deleting a tuple from an extendable hash table:
    ...
    3. merge buckets if possible:
        - if buckets are adjacent and have the *same* $i$-values, they can be merged together
        ...
        - update the directory pointer to the merged buckets
...
- is there a minimum space guarantee for extendable hash tables?
    - unlike in B+ trees, there is no flexibility of where to split:
        ...
        - cannot choose to always promote the middle key
...
- comparing with B+ tree:
    - for a range query such as `SELECT * FROM R WHERE R.A>5`:
        ...
        - while in a B+ tree, the range query does well since the leaves can be traversed in order
    - for a point query such as `SELECT * FROM R WHERE R.A=5`:
        - extendable hash excels at point searches, has constant access of at most 2 disk IOs
        - B+ tree still does well on the order of the height of the tree
    - the efficiency of a B+ tree is quite competitive considering the fanout factor is very large:
    ...
    - thus, B+ trees are the typical choice for a default indexing structure since they have good overall performance
...
- in the **nested loop join (NLJ)** algorithm:
    ...
    - compare each element of $R$ against each element of $S$:
        ...
        - requiring an *additional* full scan of the *other* table eg. $S$ is the most expensive operation of the algorithm
...
- in a **hash join (HJ)**:
    - given a hash function $h$, can two tuples $r, s$ join if $h(r.A) \neq h(s.A)$?
        - no, they cannot join since the joining attribute is different
...
- how do we judge performance of the different join algorithms?
    - need a **cost model** to estimate the performance of a join algorithm:
        - eg. one model number of disk blocks read or written during a join, ignoring the last IO for writing the final result
...
- cost of sort-merge join:
    - in the join stage, after tables are sorted, we have two pointers to each table:
        - however, when reading and writing, must be done in a block rather than a tuple granularity
            - need to stage the output in a cache block as well until it can be written out to disk (ignored in cost model)
...
- cost of nested loop join:
    ...
    - however, we can do even better by using the remaining blocks in memory:
        ...
        - altogether, this version reads $b_R + \lceil\frac{b_R}{M-2}\rceil \times b_S$ blocks or $100 + \lceil\frac{100}{20}\rceil \times 1000 = 5100$ blocks in our example
            - only factor of 5 difference from sort-merge
...
- cost of hash join:
    - partitioning stage:
        ...
        - however, we need to also need to write out the buckets
            - assuming perfect hashing, this requires $\lceil \frac{\textit{blocks}}{M-1} \rceil \times (M-1)$
            - in general $2 \times \textit{blocks}$, since the outputted buckets have approximately the same number of blocks that are read in
        - in general, $2 \times b_R + 2 \times b_S$ disk IOs are performed or $2000 + 200 + 2200$ disk IOs in our example
    - joining stage:
    ...
    - overall, we have $2 \times (b_R + b_S) + (b_R + b_S) = 3 \times (b_R+b_S)$ or $3 \times (1100) = 3300$ disk IOs in our example
        - generally, for every block, is read and write in partitioning stage, and then read in joining stage
