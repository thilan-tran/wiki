...
- ex. What do the relationships in Figure \ref{notex} mean?
    ...
    - every student must take a class and every class must be taken by students
...
- sometimes, it is necessary to have more than just a binary relationship:
    ...
    - eg. as seen in Figure \ref{tertex}:
...
\begin{tikzpicture}
    ...
    \node[shape=circle,draw=black,scale=0.5] (pnum) at (6,-2) {ProjectNum};
...
- a **weak entity set** is one without a unique key:
    ...
    - eg. as seen in Figure \ref{weakex}, a project report is submitted by a student but only has a project number attribute
...
Take(PRIMARY KEY quarter, PRIMARY KEY Student.name,
     PRIMARY KEY TA.name, PRIMARY KEY cnum)
Partner(PRIMARY KEY coder, tester)
ProjectReport(PRIAMRY KEY num, grade, PRIMARY KEY name)
...
- ex. Consider the following table design:
    ...
    - should this be split into multiple entities?
        - yes, we can see that information correspondign to students and classes will both be redundantly represented in the design
...
- is there an algorithm to arrive at the better design more systematically?
    ...
    - uses the concept of **functional dependency (FD)** from normalization theory:
    ...
    - when there is a functional dependency due to this determination, we may have redundancy
        - can decompose these tables into smaller ones to reduce redundancy
