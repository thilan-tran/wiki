...
- the ALU control specifies its different functions:
    ...
    - for load and stores, need to function as an add for effective address computation
        - note that this hard-wired addition operation is set by a different `ALUOp` control than the other possible R-type addition, since loads and store instructions do not have space to specify a `funct` field
