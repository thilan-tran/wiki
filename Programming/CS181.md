...
for a regular language $A$, $\bar{A}$ is also a regular language:

- for $A$, we have a DFA $M = (Q,\Sigma,\delta,q,F)$ that accepts $A$:
    - a DFA $M'$ that accepts $\bar{A}$ can be constructed by setting $M' = (Q,\Sigma,\delta,q,(Q-F))$
...
- non-closure properties for CFLs (unlike FSLs):
    ...
    - complementation:
        ...
        - $S\to\enskip A\enskip|\enskip B\enskip|\enskip AB\enskip|\enskip BA$ is a CFG for $\bar{L_3}$
...
- a reduction from $u$ to $v$ is a sequence of reductions:
