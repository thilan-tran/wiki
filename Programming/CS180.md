...
- in a **stable matching**, there are no unstable matches for any two pairs in the matching
- find a **stable matching**

*Solution*:

1. *repeatedly*, pick an arbitrary person $p$ from the first group who is unmatched
  - go down their preference list starting at the highest rank that hasn't yet been asked (this solution never goes *backwards* in the list)
...
- instead, maintain a *count* of votes for a current candidate
  - linearly go through the votes, *incrementing* the count if a vote for the current candidate, otherwise *decrementing*
...
- *claim*: removing two different votes *maintains* a majority
  ...
  - if two different votes are removed, the ratio becomes $\frac{n}{2}$ out of $n-2$ votes
    - if one vote for the majority was removed, otherwise there are still $\frac{n}{2}+1$ votes for the majority
...
Another proof method is finding a structural *bound* that eery optimal solution would have.
...
- if the *frequencies* of letters ie. encoded data is given, the tree can further optimized:
  - letters that have a higher frequency should have a *shorter* encoding so that less memory is used
  - ie. *minimize* the function $\sum_{i=1}^n l_i f_i$ where $f_i$ is a letter's frequency and $l_i$ is its encoded length in the tree code
...
- graphs can be *represented* in different ways:
  - using a matrix or 2D-array
    ...
    - *cons*:
      ...
      - more difficult to check all adjacencies on a particular vertices
...
- a **BFS tree** can be formed out of the edges used to visit a vertex for the *first* time
...
- note that DFS can be useful for finding cycles:
  - if a previously visited neighbor is found while backtracking through new nodes, a cycle exists

- a **DFS tree** can be formed from all the edges used to *discover* a node
...
- DFS tree *properties*:
  ...
  - *claim*: if the edge $(x, y) \in G$ but $(x, y) \notin$ the DFS tree, then one of $x$ or $y$ is an ancestor of the other
    - an edge will not be in the DFS tree if the ending nodes of the edge were already explored
...
- find if a graph is 2-colorable, or can be colored in 2 colors
...
- only a BFS and possibly a second linear traversal through the graph
...
- to efficiently implement this algorithm, we want to minimize searching for source nodes
  1. count all incident edges to all nodes *before* the iteration starts - $O(m)$
...
1. start at a root vertex $s$
  - place $s$ in the set of vertices with *finalized* minimum distances from $s$, ie. the minimum distance from $s$ to $s$ is 0
2. *repeatedly*, grow the set of finalized vertices:
  - find the unfinalized vertex $v$ with minimum distance, and add it to the set
    - then, *relax* the distance to each of its neighbors $u$ as follows:
      - $d(v) = min(d(v), d(u) + l(e))$, where $e = (v, u)$
...
- there are two tpyes of nodes that have to be maintained throughout the algorithm
...
- at each iteration:
  - to find the next minimum neighbor, just check all non-finalized neighbors - $O(n)$
  - then, update the distances and reorganize all adjacent vertices to the minimum neighbor - $O(n)$
...
- for a graph $G$, let $w_{\textit{min}}$ be the minimum weight edge between two partitions of $G$, $L$ and $R$
  - *MST theorem*: for an MST $T$ on $G$, $w_{\textit{min}} \in T \forall$ bipartitions of $G$
...
2. *repeatedly*, at each step, add the node that can be attached the cheapest (different *relax* function)
...
- actual implementation is more complex than reusing Dijkstra's
- have to maintain connected components so that it is easy to check for cycles when inserting edges
  - need a data structure to efficiently *merge* components and find the *identity* of a component
    - called the **union-find** data structure, complexity depends on the operation of this structure
...
- desired operations:
  ...
  - **find** or queries: are two elements part of the same group
...
- also maintain an array for every group of all of its elements
...
- operation complexities:
  ...
  - find - $O(logn)$
  - union - $O(1)$
    - the usage pattern is usually a find operation, then a union operation having found the root ie. name of the group
...
- given graph $G$ and $k$ desired clusters, place nodes close to each other in the same cluster and nodes far away in different clusters
  ...
  - the **distance** between two clusters is the shortest distance (ie. weight of edge) between two nodes in each cluster, ie. the closest pair
- *maximize* the minimum distance, ie. spread out clusters as much as possible

*Solution*:

- another algorithm that utilized a form of clusetring was Kruskall's
...
- if a DAG $G$ has a longest path of length $k$, partition $G$'s vertices into $k+1$ groups such as there are no edges and no paths between each group
...
- by the definition of a source, there are no edges between sources, so the condition of independent groups is satisfied
  - in addition, if $k$ is the longest path, removing all sources at a time would reduce the path by 1 and allow for $k+1$ groups
...
- use an adapted DFS that keeps track of $Low(v)$, the smallest value of a subtree that is accessible by a backedge
...
- the **divide and conquer** paradigm is a problem-solving approach where we break a problem into multiple pieces, and reform the pieces after
  - dividng should be *easy* and done recursively
    - should reduce the problem to a trivial solution
...
2. *repeatedly*, compare the pointers, put the smaller element into the output list, and incrememt that pointer
...
1. initialize pointers to the start of each list, and initialize the number of crossings to the number of crossings in each half
2. *repeatedly*:
  - compare the pointers $L_i$ and $R_j$
  - put the smaller element into the output list
  - whenever $R_j < L_i$, we know there are a number of crossings equal to $S$, the *remaining* elements in $L$ after $L_j$
    - ie. since $L$ and $R$ are sorted, we can add $S$ to the count in one go
...
- find the *closest* pair among a set of coordinates
  ...
  - $O(nlogn)$ - can *sort* the popints by an axis, or try divide and conquer with a linear merging algorithm
...
- assume that we have halves $L$ with coordinates $x_i$ and $R$ with coordinates $y_j$, and that $\delta_L$ and $\delta_R$ are the closest distances in each half
  ...
  - note that there cannot be *too many* $y$'s to compare, because then the smallest distance would be one of the pairs in $R$
  ...
  - then, we can additionally limit along the y-axis for every $x$ we are comparing with $y$'s:
    - can check coordinates a distance $\delta$ above and below $x$'s y position
    - this creates 16 square boxes each of length $\frac{\delta}{2}$
  - note that the number of points in each box is *at most* 1:
    - if there were two points, they would have a maximum distance from each other along the diagonal of the box of $\frac{\sqrt{2}}{2}\delta$
      - this distance is less than $\delta$, which breaks the assumption since each box is entirely in either $L$ or $R$
  - thus there are *at most* 16 total $y$'s to compare with each $x$ for potentially closer pairs
...
3. merge the solutions together:
  - to calculate the minimum distance between halves:
    - for all $x$ in one half, compare $x$ with at most 16 neighboring points using the coordinates sorted by y-axis
...
- let $\textit{OPT}[l, r]$ represent the total weight of the optimal set of intervals that can be selected between time $l$ and $r$
1. *repeatedly*, grow $\textit{OPT}$ using the following **transition**, ie. relationship between iterations:
  - $\textit{OPT}[0, r] = \textit{max}(\textit{OPT}[0, l-1] + W_i, \textit{OPT}[0, r-1])$
    - where $W_i$ is the weight of current interval we are considering, with start and end times $l$ and $r$
  - we can either *include* or *exclude* each interval $i$ from our solution:
    - $\textit{OPT}[0, l-1] + W_i$ is the total weight possible from including the interval
    - $\textit{OPT}[0, r-1]$ is the total weight possible from excluding the interval
  - the algorithm terminates when $\textit{OPT}[0, t]$ is found, where $t$ is the ending time of the latest interval
  - note that although $\textit{OPT}$ is written in 2D, the first dimension representing the starting time is fixed to time 0, so it is really a 1D transition
...
- the algorithm has three stages:
  ...
  - retreve full solution using pointers - $O(n)$
...
- considerations:
  1. if all items are of the same size, trivial solution is to take all items with the greatest $v_i$ until knapsack is full
...
- need a **parameter** for DP to transition over, similar to the time axis in interval scheduling
  ...
  - can use both, 2D $\textit{OPT}$
- let $\textit{OPT}[i, j]$ represent the maximum value subsolution for $i$ items considered and a capacity $j$
- the order in which we transition over $\textit{OPT}$ should be consistent
  - eg. traverse through $\textit{OPT}$ in row-major order until $\textit{OPT}[n, S]$, our desired solution
    - could also use column-order
1. *repeatedly*, grow $\textit{OPT}$ using the following transition:
  ...
  - we can either *include* or *exclude* each item $i$ from our solution:
    - $\textit{OPT}[i, j-s_i] + v_i$ is the total value possible from including the item
      ...
      - filling out the rest of the knapsack from a previous subsolution
...
1. *repeatedly*, grow $\textit{OPT}$ using the following transition:
  - for each of the neighbors of $x$ $\{y_1, ..., y_d\}$:
...
- the transition consists of several parameters:
  ...
  - iterating over all nodes $x$ in the graph - $O(n)$
    - alternatively, instead of accounting for each neighbor and each node, the algorithm considers each edge exactly once
...
- let $\textit{OPT}[i, j]$ represent the optimal number of matches for the RNA sequence subinterval between $i$ and $j$
  - need $i$ as another parameter to track matches *starting* at indices that are not 0
    - this allows matches to be nested
1. *repeatedly*, grow $\textit{OPT}$ using the following transition:
  - $\textit{OPT}[i, j] = \textit{max}(\textit{OPT}_{\textit{include}}, \textit{OPT}_{\textit{exclude}})$
    - where $\textit{OPT}_{\textit{include}} = \textit{OPT}[i,k-1]+\textit{OPT}[k+1,j-1]+1$
...
- the transition consists of several parameters:
  - iterating over all possible matches $k$ with a single base - $O(n)$
...
- other definitions and related theorems:
  ...
  - the maximum flow on a graph is *equal* to the **minimum cut** of the graph
...
- each iteration consists of a DFS and then a linear operation on the path found
  - the flow is also *strict* increased at iteration, since every new path contributes a new flow out of $s$ of at least 1 unit
...
1. *repeatedly*, delete all leaves
  ...
  - continue until no vertices remain
- this specific version of the solution is correct, and runs i n$O(n)$
...
3. **3-satisfiability problem (3SAT)**:
  - given a *boolean* equation, can we assign 0 and 1's such that the equation $= 1$
    - eg. $F = (x_1 + \overline{x_2} + x_3) \cdot (x_1 + x_4 + \overline{x_5}) \cdot ...$
      ...
      - equation is a series of and's of or's of literals
