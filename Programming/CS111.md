...
- early solution, the **file system checker** or `fsck`:
  - allow inconsistencies to occur, and then fix them later (eg. at reboot)
...
- other journaling *optimizations*:
  ...
  - treat the log as a **circular log** so that it can be reused:
    - once a transaction has been successful checkpointed, the file system should *free* this log space from the journal
...
- *motivations*:
  ...
  - FFS performas poorly for even common workloads such as creating a small file (lots of metadata to update)
...
- finding inodes:
  ...
  - introduce a layer of **indirection** with a **inode map**
  ...
  - placing the imap on disk:
    ...
    - instead, place *chunks* of the imap right where all the other updates are being written
  ...
  - finding the imap:
    ...
    - fixed place is the **checkpoint region (CR)**:
      - contains pointers to the *latest* chunks of the inode map (spread out over disk)
...
- **garbage collection**:
  ...
  - could maintain old versions in a **versioning** system
...
- *general* OS goals should be converted into more *specific* policies that can actually be implemented
  ...
  - the mechanisms then must be *applied* to specific policies in order to help satisfy a security goal
...
- *attaching* identities to processes:
  - when a process is created, check the *creating* processes control block for its identity, and copy over the indentity
    - eg. on a `fork`
...
- using **access control lists (ACL)** for access control:
  ...
  - tried-and-true UNIX solution:
    ...
    - no room to for what user the entry pertains to?
      - partition the ACL into three set *categories*:
  ...
  - *cons*:
    - have to store ACL near file and deal with long ACL
    ...
    - difficulties translated user name spaces across different domains, eg. different web servers
...
- using **capabilities** for access control:
  ...
  - some systems use both:
    ...
    - instead a data structure amounting to a capability is attached to the PCB, that is checked instead
...
- when data is sent between machines, want to preserve:
  ...
  - **privacy** of data
- cryptography helps will all of these goal
...
- **asymmetric** or **public key** cryptography:
  ...
  - encrypting with secret key allows *authentication*, while encrypting with public key allows *secret communication*
    - for both authentication and private communication, multiple key pairs are required
      - A encrypts with A's private key to authenticate only A could have created the message
